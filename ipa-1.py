...
def savings(gross_pay, tax_rate, expenses):
    ...
    after_tax = (float(gross_pay) * tax_rate) 
    net_pay = gross_pay - after_tax
    minus_expenses = net_pay - expenses
...
def material_waste(total_material, material_units, num_jobs, job_consumption):
    ...
    available_minus_consumed = total_material - total_material_consumed
    waste = str(available_minus_consumed)+material_units
    return waste

print(material_waste(5000,"kg",3,50))
...
def body_mass_index(weight, height):
    ...
    feet_m = float(height[0]) * 0.3048
    inches_m = float(height[1]) * 0.0254
    height_m = feet_m + inches_m
